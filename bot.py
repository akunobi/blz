...
_TOKEN_WHITELIST = frozenset([
    "retardant", "retardation", "cockpit", "cocktail", "cockatoo", "cooker", "cockroach",
    "dickens", "dickinson", "dickson", "heiling", "hellongjiang", "cumulative", "cumulatively",
    "cumbia", "cumulonimbus", "cummings", "cumulus",
])

_EXACT_ONLY = frozenset(["spic", "cock", "dick", "cum", "gay", "kys", "lag", "hell"])

_BAD_SINGLE = [
    "nigger", "nigga", "niga", "negger", "laggor", "lagot", "laget", "lag", "retard",
    "chink", "spic", "webback", "kike", "gook", "coon", "jigabo", "beamer", "tranny",
    "pom", "prom", "cumshot", "blowjob", "handjob", "pusay", "cunt", "cum", "cock", "cocksucker",
    "dick", "dickhead", "kys", "nazi", "kik", "hell", "bitch", "asshole", "motherfucker", "cuck", "gay",
]

_BAD_PHRASES = [
    "kill yourself", "kill your self", "hang yourself", "hang your self", "white power", "hell hitler",
]
...
def contains_bad_word(text: str):
    ...
    for phrase in _BAD_PHRASES:
        pn = _norm(phrase)
        pat = r"(\*|%20-30|#)" + _re_mod.escape(pn) + r"(|a-20-9|)"
...
def add_warning(user_id, user_name, guild_id, reason, mod_id="BOT", mod_name="AutoMod",
    ...
    conn = get_db_connection()
    conn.execute('''
        INSERT INTO warnings (user_id, user_name, guild_id, reason, message_content, message_link, moderator_id, moderator_name)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
def log_mod_action(user_id, user_name, guild_id, action, reason, duration=None, mod_id="BOT", mod_name="AutoMod"):
    conn = get_db_connection()
    conn.execute('''
        INSERT INTO mod_actions (user_id, user_name, guild_id, action, reason, duration_seconds, moderator_id, moderator_name)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
def get_recent_mod_action(user_id, guild_id, action):
    ...
    ''', (str(user_id), str(guild_id), action)).fetchone()
    conn.close()
...
async def notify_warn_admins(member, guild, warn_count):
    ...
    for admin_id in WARN_DM_ADMIN_IDS:
        try:
            ...
            embed = discord.Embed(
                title="⚠️ User has 2 warnings",
                description=f"**{member.display_name}** ({member.id}) in **{guild.name}** now has **{warn_count} warnings**.",
                color=0xF5A623
            )
...
async def on_message(message):
    ...
    if guild and message.content:
        ...
        norm_content = message.content.lower().strip()
        h = hashlib.md5(norm_content.encode()).hexdigest() if norm_content else ''
                        ...
                        'components': [
                            {
                                ...
                                'label': c.label,
                                'style': c.style.value if hasattr(c.style, 'value') else None,
                                'emoji': c.emoji.name if c.emoji else None,
                                'id': str(c.emoji.id) if c.emoji and c.emoji.id else None,
                                'url': c.url,
                                'disabled': c.disabled,
...
async def _openai_moderate(message):
    ...
    if top_cat in _MOD_LOG_ONLY and top_cat not in _MOD_CATEGORIES:
        log_mod_action(message.author.id, message.author.name, guild.id, 'ai_flagged_log', f'AI log-only: {top_cat} ({top_score:.0%})')
        return
    try:
        await message.delete()
    except Exception:
        pass
    msg_link_ai = f"https://discord.com/channels/{guild.id}/{message.channel.id}/{message.id}"
    warn_count = add_warning(
        message.author.id, message.author.name, guild.id,
        reason=f'AI: {label} ({top_score:.0%})',
        message_content=text[:500],
        message_link=msg_link_ai
    )
    log_mod_action(message.author.id, message.author.name, guild.id, 'ai_automod', f'AI: {top_cat} ({top_score:.0%}) | msg: {text[:120]}')
    try:
        await message.channel.send(f'{message.author.mention} Tu mensaje fue eliminado: "{label}". Advertencia {warn_count}.', delete_after=8)
    except Exception:
        pass
    member = guild.get_member(message.author.id)
    if member:
        await escalate_user(member, guild, warn_count, f'AI: {top_cat}')
